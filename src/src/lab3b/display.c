...
#define COL_ERROR_BAR 0377, 0333, 0333
...
#define RECT_ERROR_BAR(COL, PER) RECTBG(COL, 0, 260, PER, 10)
...
void update_octave(void){
	u32 a = lab3b.octave;
	setFont(BigFont);
	printChar(a < 10 ? a + 0x30 : 'A', 120-8, 150);
	setFont(SmallFont);
	printChar(a > 0 ? a + 0x2F : 'A', 120-24, 150);
	printChar(a < 9 ? a + 0x31 : a < 10 ? 'A' : '0', 120+16, 150);
}
...
void draw_tuner(void){
	...
	lcdPrint("Hz",120+8,170);
	update_tuner();
}

void update_tuner(void){
	RECT_ERROR_BAR(COL_BG, 240)
	setColorBg(COL_TUNER);
	...
	printChar((freq / 1000) ? (freq / 1000 + 0x30) : ' ' , 120-24, 170);
	printChar((freq%1000)/100 + 0x30, 120-16, 170);
	printChar((freq%100)/10 + 0x30, 120-8, 170);
	printChar(freq%10 + 0x30, 120, 170);

	//Error Bar
	int error = note % 100*12/5;
	RECT_ERROR_BAR(COL_ERROR_BAR, error)
